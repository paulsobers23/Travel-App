const getTrips = async (origin, departureDate, returnDate, budgetPrice) => {
  ...
  const flights = await getRoundTripFlight(origin, destination, departureDate, returnDate, budgetPrice);
  if (flights) {
    const roundTripPrice = flights[0].Dates.OutboundDates[0].Price + flights[1].Dates.OutboundDates[0].Price;
    if (roundTripPrice < budgetPrice) {
      // console.log(roundTripPrice, topForty[origin], topForty[destination])
      const arrHotels = await getCityByLatLng(topForty[destination]);
      const hotelPriceStr = await getMinPrice(arrHotels);
      const hotelPriceNum = parseInt(hotelPriceStr.substring(1));
      console.log(`lowest flight price:${roundTripPrice} lowest hotel price:${hotelPriceNum}`);
      const totalPrice = hotelPriceNum + roundTripPrice;
      if (totalPrice <= budgetPrice) {
        return {
          hotels: arrHotels, roundTrip: flights, lowestPrice: totalPrice, origin, destination,
        };
      }
      return null;
    }
